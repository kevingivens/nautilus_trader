...
from nautilus_trader.adapters.coinbase.enums import CoinbaseEnumParser
from nautilus_trader.adapters.coinbase.enums import CoinbaseExchangeFilterType
from nautilus_trader.adapters.coinbase.enums import CoinbaseKlineInterval
from nautilus_trader.adapters.coinbase.enums import CoinbaseRateLimitInterval
from nautilus_trader.adapters.coinbase.enums import CoinbaseRateLimitType
...
from nautilus_trader.adapters.coinbase.types import CoinbaseTicker
...
class CoinbaseTime(msgspec.Struct, frozen=True):
    """
...
class CoinbaseDepth(msgspec.Struct, frozen=True):
    """
...
class CoinbaseTrade(msgspec.Struct, frozen=True):
    ...
    def parse_to_trade_tick(
        ...
        """Parse Coinbase trade to internal TradeTick."""
        return TradeTick(
            instrument_id=instrument_id,
...
class CoinbaseAggTrade(msgspec.Struct, frozen=True):
...
class CoinbaseKline(msgspec.Struct, array_like=True):
    """Array-like schema of single Coinbase kline."""
...
class CoinbaseTicker24hr(msgspec.Struct, frozen=True):
    """Schema of single Coinbase 24hr ticker (FULL/MINI)."""
...
class CoinbaseTickerPrice(msgspec.Struct, frozen=True):
    """Schema of single Coinbase Price Ticker."""
...
class CoinbaseTickerBook(msgspec.Struct, frozen=True):
    """Schema of a single Coinbase Order Book Ticker."""
...
class CoinbaseDataMsgWrapper(msgspec.Struct):
    """
    Provides a wrapper for data WebSocket messages from `Coinbase`.
    """

    stream: str


class CoinbaseOrderBookDelta(msgspec.Struct, array_like=True):
...
class CoinbaseOrderBookData(msgspec.Struct, frozen=True):
    """WebSocket message 'inner struct' for `Coinbase` Partial & Diff. Book Depth Streams."""
    ...
    b: list[CoinbaseOrderBookDelta]  # Bids to be updated
    a: list[CoinbaseOrderBookDelta]  # Asks to be updated
...
class CoinbaseOrderBookMsg(msgspec.Struct, frozen=True):
    """WebSocket message from `Coinbase` Partial & Diff. Book Depth Streams."""

    stream: str
    data: CoinbaseOrderBookData


class CoinbaseQuoteData(msgspec.Struct, frozen=True):
    """WebSocket message from `Coinbase` Individual Symbol Book Ticker Streams."""
...
class CoinbaseQuoteMsg(msgspec.Struct, frozen=True):
    """WebSocket message from `Coinbase` Individual Symbol Book Ticker Streams."""

    stream: str
    data: CoinbaseQuoteData


class CoinbaseAggregatedTradeData(msgspec.Struct, frozen=True):
    """WebSocket message from `Coinbase` Aggregate Trade Streams."""
...
class CoinbaseAggregatedTradeMsg(msgspec.Struct, frozen=True):
    """WebSocket message."""

    stream: str
    data: CoinbaseAggregatedTradeData


class CoinbaseTickerData(msgspec.Struct, kw_only=True, frozen=True):
    """
    WebSocket message from `Coinbase` 24hr Ticker
...
class CoinbaseTickerMsg(msgspec.Struct, frozen=True):
    """WebSocket message."""

    stream: str
    data: CoinbaseTickerData


class CoinbaseCandlestick(msgspec.Struct, frozen=True):
    """
    WebSocket message 'inner struct' for `Coinbase` Kline/Candlestick Streams.
    ...
    s: str  # Symbol
    i: CoinbaseKlineInterval  # Interval
    f: int  # First trade ID
    L: int  # Last trade ID
    o: str  # Open price
    c: str  # Close price
...
class CoinbaseCandlestickData(msgspec.Struct, frozen=True):
    """WebSocket message 'inner struct'."""

    e: str
    E: int
    s: str
    k: CoinbaseCandlestick


class CoinbaseCandlestickMsg(msgspec.Struct, frozen=True):
    """WebSocket message for `Coinbase` Kline/Candlestick Streams."""

    stream: str
    data: CoinbaseCandlestickData
