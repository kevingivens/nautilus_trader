...
from nautilus_trader.adapters.coinbase.enums import CoinbaseOrderStatus
...
class CoinbaseUserTrade(msgspec.Struct, frozen=True):
    """
    HTTP response from `Coinbase`
        `GET /api/v3/myTrades`
    """
    ...
    # Parameters not present in 'fills' list (see FULL response of CoinbaseOrder)
...
class CoinbaseOrder(msgspec.Struct, frozen=True):
    """
    HTTP response from `Coinbase`
        `GET /api/v3/brokerage/orders`
    """
    ...
    type: Optional[CoinbaseOrderType] = None
    side: Optional[CoinbaseOrderSide] = None
    ...
    fills: Optional[list[CoinbaseUserTrade]] = None  # FULL response only

    # Parameters in FUTURES only:
    avgPrice: Optional[str] = None
    origType: Optional[CoinbaseOrderType] = None
    ...
    def parse_to_order_status_report(
        ...
        if self.price is None:
            raise RuntimeError(
                "Cannot generate order status report from Coinbase ACK response.",
            )
        ...
        post_only = (
            self.type == CoinbaseOrderType.LIMIT_MAKER or self.timeInForce == CoinbaseTimeInForce.GTX
        )
        ...
        return OrderStatusReport(
            ...
            venue_order_id=VenueOrderId(str(self.orderId)),
            order_side=enum_parser.parse_coinbase_order_side(self.side),
            order_type=enum_parser.parse_coinbase_order_type(self.type),
            contingency_type=contingency_type,
            time_in_force=enum_parser.parse_coinbase_time_in_force(self.timeInForce),
            order_status=enum_parser.parse_coinbase_order_status(self.status),
...
class CoinbaseStatusCode(msgspec.Struct, frozen=True):
    """
