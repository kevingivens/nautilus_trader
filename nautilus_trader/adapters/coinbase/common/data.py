...
from nautilus_trader.adapters.coinbase.schemas.market import CoinbaseAggregatedTradeMsg
from nautilus_trader.adapters.coinbase.schemas.market import CoinbaseCandlestickMsg
from nautilus_trader.adapters.coinbase.schemas.market import CoinbaseDataMsgWrapper
from nautilus_trader.adapters.coinbase.schemas.market import CoinbaseOrderBookMsg
from nautilus_trader.adapters.coinbase.schemas.market import CoinbaseQuoteMsg
from nautilus_trader.adapters.coinbase.schemas.market import CoinbaseTickerMsg
...
from nautilus_trader.adapters.coinbase.websocket.client import CoinbaseWebSocketClient
...
class CoinbaseCommonDataClient(LiveMarketDataClient):
    """
    Provides a data client of common methods for the `Coinbase` exchange.
    ...
    market : CoinbaseMarketHttpAPI
        The binance Market HTTP API.
    enum_parser : CoinbaseEnumParser
        The parser for Coinbase enums.
    msgbus : MessageBus
    ...
    use_agg_trade_ticks : bool, default False
        ...
        TradeId of ticks will be the Aggregate tradeId returned by Coinbase.
    ...
    def __init__(
        ...
        self._decoder_data_msg_wrapper = msgspec.json.Decoder(CoinbaseDataMsgWrapper)
        self._decoder_order_book_msg = msgspec.json.Decoder(CoinbaseOrderBookMsg)
        self._decoder_quote_msg = msgspec.json.Decoder(CoinbaseQuoteMsg)
        self._decoder_ticker_msg = msgspec.json.Decoder(CoinbaseTickerMsg)
        self._decoder_candlestick_msg = msgspec.json.Decoder(CoinbaseCandlestickMsg)
        self._decoder_agg_trade_msg = msgspec.json.Decoder(CoinbaseAggregatedTradeMsg)
    ...
    async def _subscribe_order_book(  # noqa (too complex)
        ...
        if book_type == BookType.L3_MBO:
            self._log.error(
                ...
                "L3_MBO data is not published by Coinbase. "
    ...
    async def _subscribe_bars(self, bar_type: BarType) -> None:
        ...
        if not bar_type.spec.is_time_aggregated():
            self._log.error(
                f"Cannot subscribe to {bar_type}: only time bars are aggregated by Coinbase.",
            )
            return
        ...
        if self._binance_account_type.is_futures and resolution == "s":
            self._log.error(
                ...
                "Second interval bars are not aggregated by Coinbase Futures.",
            )
        try:
            interval = CoinbaseKlineInterval(f"{bar_type.spec.step}{resolution}")
        except ValueError:
            self._log.error(
                f"Bar interval {bar_type.spec.step}{resolution} not supported by Coinbase.",
            )
            return
    ...
    async def _unsubscribe_order_book_deltas(self, instrument_id: InstrumentId) -> None:
        pass  # TODO: Unsubscribe from Coinbase if no other subscriptions
    ...
    async def _request_quote_ticks(
        ...
        self._log.error(
            "Cannot request historical quote ticks: not published by Coinbase.",
        )
    ...
    async def _request_bars(  # noqa (too complex)
        ...
        if bar_type.is_internally_aggregated():
            self._log.error(
                ...
                f"only historical bars with EXTERNAL aggregation available from Coinbase.",
            )
            return

        if not bar_type.spec.is_time_aggregated():
            self._log.error(
                f"Cannot request {bar_type}: only time bars are aggregated by Coinbase.",
            )
            return
        ...
        try:
            ...
            self._log.error(
                f"Cannot create Coinbase Kline interval. {bar_type.spec.step}{resolution} "
                "not supported.",
            )
            return
        ...
        if bar_type.spec.price_type != PriceType.LAST:
            self._log.error(
                f"Cannot request {bar_type}: "
                f"only historical bars for LAST price type available from Coinbase.",
            )
            return
    ...
    def _get_cached_instrument_id(self, symbol: str) -> InstrumentId:
        ...
        if not instrument_id:
            instrument_id = InstrumentId(Symbol(nautilus_symbol), COINBASE_VENUE)
