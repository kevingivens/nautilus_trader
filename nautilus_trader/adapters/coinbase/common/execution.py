...
from nautilus_trader.adapters.coinbase.common.enums import CoinbaseEnumParser
from nautilus_trader.adapters.coinbase.common.enums import CoinbaseTimeInForce
from nautilus_trader.adapters.coinbase.common.schemas.account import CoinbaseOrder
from nautilus_trader.adapters.coinbase.common.schemas.account import CoinbaseUserTrade
from nautilus_trader.adapters.coinbase.common.schemas.symbol import CoinbaseSymbol
from nautilus_trader.adapters.coinbase.common.schemas.user import CoinbaseListenKey
from nautilus_trader.adapters.coinbase.http.account import CoinbaseAccountHttpAPI
from nautilus_trader.adapters.coinbase.http.client import CoinbaseHttpClient
from nautilus_trader.adapters.coinbase.http.error import CoinbaseError
...
class CoinbaseExecutionClient(LiveExecutionClient):
    """
    Execution client providing functionality for the `Coinbase` exchanges.
    ...
    def __init__(
        ...
        self._set_account_id(AccountId(f"{COINBASE_VENUE.value}-spot-master"))
    ...
    async def _connect(self) -> None:
        ...
        try:
            ...
            # Get listen keys
            response: CoinbaseListenKey = await self._http_user.create_listen_key()
        except CoinbaseError as e:
            self._log.exception(f"Error on connect: {e.message}", e)
            return

        # Check Coinbase-Nautilus clock sync
        server_time: int = await self._http_market.request_server_time()
        self._log.info(f"Coinbase server time {server_time} UNIX (ms).")
    ...
    async def generate_order_status_reports(
        ...
        try:
            # Check Coinbase for all order active symbols
    ...
    async def generate_trade_reports(
        ...
        try:
            # Check Coinbase for all trades on active symbols
            symbol = instrument_id.symbol.value if instrument_id is not None else None
            active_symbols = self._get_cache_active_symbols()
            active_symbols.extend(await self._get_binance_active_position_symbols(symbol))
            binance_trades: list[CoinbaseUserTrade] = []
            for symbol in active_symbols:
    ...
    async def _submit_limit_order(self, order: LimitOrder) -> None:
        ...
        if order.time_in_force == TimeInForce.GTD and time_in_force == CoinbaseTimeInForce.GTC:
        ...
        if order.is_post_only and self._binance_account_type.is_spot_or_margin:
            ...
            time_in_force = CoinbaseTimeInForce.GTX
    ...
    async def _submit_trailing_stop_market_order(self, order: TrailingStopMarketOrder) -> None:
        ...
        if not activation_price:
            ...
            if quote:
                ...
                self._log.error(
                    "Cannot submit order: no trigger price specified for Coinbase activation price "
    ...
    def _get_cached_instrument_id(self, symbol: str) -> InstrumentId:
        # Parse instrument ID
        nautilus_symbol: str = CoinbaseSymbol(symbol).parse_binance_to_internal(
