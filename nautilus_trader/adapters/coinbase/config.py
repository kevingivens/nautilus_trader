...
class CoinbaseDataClientConfig(LiveDataClientConfig, frozen=True):
    ...
    api_key : str, optional
        ...
        If ``None`` then will source the `COINBASE_API_KEY` or
        `COINBASE_TESTNET_API_KEY` environment variables.
    api_secret : str, optional
        The Coinbase API public key.
        If ``None`` then will source the `COINBASE_API_KEY` or
        `COINBASE_TESTNET_API_KEY` environment variables.
    account_type : CoinbaseAccountType, default CoinbaseAccountType.SPOT
    ...
    us : bool, default False
        If client is connecting to Coinbase US.
    testnet : bool, default False
        If the client is connecting to a Coinbase testnet.
