...
from nautilus_trader.adapters.coinbase.execution import CoinbaseCommonExecutionClient
...
from nautilus_trader.adapters.coinbase.enums import CoinbaseSpotEventType
from nautilus_trader.adapters.coinbase.http.account import CoinbaseSpotAccountHttpAPI
from nautilus_trader.adapters.coinbase.http.market import CoinbaseSpotMarketHttpAPI
from nautilus_trader.adapters.coinbase.http.user import CoinbaseSpotUserDataHttpAPI
from nautilus_trader.adapters.coinbase.providers import CoinbaseSpotInstrumentProvider
from nautilus_trader.adapters.coinbase.schemas.account import CoinbaseSpotAccountInfo
from nautilus_trader.adapters.coinbase.schemas.user import CoinbaseSpotAccountUpdateWrapper
from nautilus_trader.adapters.coinbase.schemas.user import CoinbaseSpotOrderUpdateWrapper
from nautilus_trader.adapters.coinbase.schemas.user import CoinbaseSpotUserMsgWrapper
...
class CoinbaseSpotExecutionClient(CoinbaseCommonExecutionClient):
    """
    ...
    instrument_provider : CoinbaseSpotInstrumentProvider
    ...
    def __init__(
        ...
        # Spot HTTP API
        self._spot_http_account = CoinbaseSpotAccountHttpAPI(client, clock, account_type)
        self._spot_http_market = CoinbaseSpotMarketHttpAPI(client, account_type)
        self._spot_http_user = CoinbaseSpotUserDataHttpAPI(client, account_type)
        ...
        self._spot_user_ws_handlers = {
            CoinbaseSpotEventType.outboundAccountPosition: self._handle_account_update,
            CoinbaseSpotEventType.executionReport: self._handle_execution_report,
            CoinbaseSpotEventType.listStatus: self._handle_list_status,
            CoinbaseSpotEventType.balanceUpdate: self._handle_balance_update,
        }

        # Websocket spot schema decoders
        self._decoder_spot_user_msg_wrapper = msgspec.json.Decoder(CoinbaseSpotUserMsgWrapper)
        self._decoder_spot_order_update_wrapper = msgspec.json.Decoder(
            CoinbaseSpotOrderUpdateWrapper,
        )
        self._decoder_spot_account_update_wrapper = msgspec.json.Decoder(
            CoinbaseSpotAccountUpdateWrapper,
        )
    ...
    async def _update_account_state(self) -> None:
        ...
        if account_info.canTrade:
            self._log.info("Coinbase API key authenticated.", LogColor.GREEN)
            self._log.info(f"API key {self._http_client.api_key} has trading permissions.")
        else:
            self._log.error("Coinbase API key does not have trading permissions.")
    ...
    def _check_order_validity(self, order: Order) -> None:
        ...
        if order.order_type not in self._spot_enum_parser.spot_valid_order_types:
            self._log.error(
                ...
                f"orders not supported by the Coinbase Spot/Margin exchange. "
        ...
        if order.time_in_force not in self._spot_enum_parser.spot_valid_time_in_force:
            self._log.error(
                ...
                f"not supported by the Coinbase Spot/Margin exchange. "
        ...
        if order.order_type == OrderType.STOP_LIMIT and order.is_post_only:
            self._log.error(
                "Cannot submit order: "
                "STOP_LIMIT `post_only` orders not supported by the Coinbase Spot/Margin exchange. "
