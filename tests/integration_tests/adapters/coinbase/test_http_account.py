...
from nautilus_trader.adapters.coinbase.common.enums import CoinbaseOrderSide
...
from nautilus_trader.adapters.coinbase.http.account import CoinbaseOrderHttp
...
class TestCoinbaseSpotAccountHttpAPI:
    def setup(self):
        # Fixture Setup
        ...
        self.api = CoinbaseAccountHttpAPI(self.client, self.clock)
    ...
    async def test_new_order_test_sends_expected_request(self, mocker):
        ...
        await endpoint.post(
            parameters=endpoint.PostParameters(
                symbol=CoinbaseSymbol("ETHUSDT"),
                side=CoinbaseOrderSide.SELL,
                type=CoinbaseOrderType.LIMIT,
                timeInForce=CoinbaseTimeInForce.GTC,
                quantity="0.01",
                price="5000",
                recvWindow=str(5000),
        ...
        assert request["url"] == "https://api.coinbase.com/api/v3/order/test"
    ...
    async def test_new_order_sends_expected_request(self, mocker):
        ...
        await self.api.new_order(
            ...
            time_in_force=CoinbaseTimeInForce.GTC,
            quantity="0.01",
            price="5000",
            recv_window="5000",
        )
        ...
        request = mock_send_request.call_args.kwargs
        assert request["method"] == "POST"
        assert request["url"] == "https://api.coinbase.com/api/v3/order"
        assert request["params"].startswith(
            "symbol=ETHUSDT&side=SELL&type=LIMIT&timeInForce=GTC&quantity=0.01&price=5000&recvWindow=5000&timestamp=",
        )
    ...
    async def test_cancel_all_open_orders_sends_expected_request(self, mocker):
        ...
        request = mock_send_request.call_args.kwargs
        assert request["method"] == "DELETE"
        assert request["url"] == "https://api.coinbase.com/api/v3/openOrders"
    ...
    async def test_query_all_orders_sends_expected_request(self, mocker):
        ...
        request = mock_send_request.call_args.kwargs
        assert request["method"] == "GET"
        assert request["url"] == "https://api.coinbase.com/api/v3/allOrders"
        assert request["params"].startswith("symbol=ETHUSDT&recvWindow=5000&timestamp=")
    ...
    async def test_query_user_trades_sends_expected_request(self, mocker):
        ...
        request = mock_send_request.call_args.kwargs
        assert request["method"] == "GET"
        assert request["url"] == "https://api.coinbase.com/api/v3/myTrades"
        assert request["params"].startswith(
    ...
    async def test_new_spot_oco_sends_expected_request(self, mocker):
        ...
        await self.api.new_spot_oco(
            ...
            stop_iceberg_qty="50",
            stop_limit_time_in_force=CoinbaseTimeInForce.GTC,
            recv_window="5000",
        )
        ...
        request = mock_send_request.call_args.kwargs
        assert request["method"] == "POST"
        assert request["url"] == "https://api.coinbase.com/api/v3/order/oco"
        assert request["params"].startswith(
    ...
    async def test_cancel_spot_oco_sends_expected_request(self, mocker):
        ...
        request = mock_send_request.call_args.kwargs
        assert request["method"] == "DELETE"
        assert request["url"] == "https://api.coinbase.com/api/v3/orderList"
        assert request["params"].startswith(
    ...
    async def test_query_spot_all_oco_sends_expected_request(self, mocker):
        ...
        request = mock_send_request.call_args.kwargs
        assert request["method"] == "GET"
        assert request["url"] == "https://api.coinbase.com/api/v3/allOrderList"
        assert request["params"].startswith(
    ...
    async def test_query_spot_all_open_oco_sends_expected_request(self, mocker):
        ...
        request = mock_send_request.call_args.kwargs
        assert request["method"] == "GET"
        assert request["url"] == "https://api.coinbase.com/api/v3/openOrderList"
    ...
    async def test_query_spot_account_info_sends_expected_request(self, mocker):
        ...
        request = mock_send_request.call_args.kwargs
        assert request["method"] == "GET"
        assert request["url"] == "https://api.coinbase.com/api/v3/account"
        assert request["params"].startswith("recvWindow=5000&timestamp=")
