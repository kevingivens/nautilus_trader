...
ETHUSDT_COINBASE = TestInstrumentProvider.ethusdt_coinbase()


@pytest.mark.skip(reason="WIP")
class TestCoinbaseSpotDataClient:
    def setup(self):
        # Fixture Setup
        ...
        self.http_client = CoinbaseHttpClient(
            clock=self.clock,
        ...
        self.provider = CoinbaseSpotInstrumentProvider(
        ...
        self.data_client = CoinbaseLiveDataClientFactory.create(
            loop=self.loop,
            client=self.http_client,
            msgbus=self.msgbus,
            cache=self.cache,
    ...
    async def test_subscribe_instruments(self, monkeypatch):
        ...
        # Assert
        btcusdt = InstrumentId.from_str("BTCUSDT.COINBASE")
        ethusdt = InstrumentId.from_str("ETHUSDT.COINBASE")
    ...
    async def test_subscribe_quote_ticks(self, monkeypatch):
        ...
        self.msgbus.subscribe(
            topic="data.quotes.COINBASE.ETHUSDT",
            handler=handler.append,
        )

        # Act
        self.data_client.subscribe_quote_ticks(ETHUSDT_COINBASE.id)

        raw_book_tick = pkgutil.get_data(
            package="tests.integration_tests.adapters.coinbase.resources.ws_messages",
        ...
        assert handler[0] == QuoteTick(
            instrument_id=ETHUSDT_COINBASE.id,
    ...
    async def test_subscribe_trade_ticks(self, monkeypatch):
        handler = []
        self.msgbus.subscribe(
            topic="data.trades.COINBASE.ETHUSDT",
            handler=handler.append,
        )

        # Act
        self.data_client.subscribe_trade_ticks(ETHUSDT_COINBASE.id)
