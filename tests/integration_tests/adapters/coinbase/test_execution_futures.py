...
from nautilus_trader.adapters.coinbase.futures.execution import CoinbaseFuturesExecutionClient
from nautilus_trader.adapters.coinbase.futures.providers import CoinbaseFuturesInstrumentProvider
...
ETHUSDT_PERP_COINBASE = TestInstrumentProvider.ethusdt_perp_coinbase()


@pytest.mark.skip(reason="WIP")
class TestCoinbaseFuturesExecutionClient:
    def setup(self):
        # Fixture Setup
        ...
        self.provider = CoinbaseFuturesInstrumentProvider(
        ...
        self.exec_client = CoinbaseFuturesExecutionClient(
            loop=self.loop,
            client=self.http_client,
            msgbus=self.msgbus,
            cache=self.cache,
    ...
    async def test_submit_market_order(self, mocker):
        ...
        mock_send_request = mocker.patch(
            target="nautilus_trader.adapters.coinbase.http.client.CoinbaseHttpClient.send_request",
        )

        order = self.strategy.order_factory.market(
            instrument_id=ETHUSDT_PERP_COINBASE.id,
