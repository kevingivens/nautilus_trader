...
class TestCoinbaseSpotExecutionClient:
    def setup(self):
        # Fixture Setup
        ...
        self.exec_client = CoinbaseExecutionClient(
            loop=self.loop,
            client=self.http_client,
            msgbus=self.msgbus,
            cache=self.cache,
    ...
    async def test_query_order(self, mocker):
        ...
        mock_query_order = mocker.patch(
            target="nautilus_trader.adapters.coinbase.spot.execution.CoinbaseSpotExecutionClient.query_order",
        )
